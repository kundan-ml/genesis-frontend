  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        const response = await axios.post(`${BACKEND_URL}/anomaly/detect/`, formData, {
          onUploadProgress: (progressEvent) => {
            const chunkProgress = Math.round(
              (progressEvent.loaded * 100) / progressEvent.total
            );
            const overallProgress = Math.round(
              ((i * CHUNK_SIZE * 100) + (chunkProgress * chunk.length)) / uploadedImage.length
            );
...
const handleDetect = async () => {
  ...
  return (
    <div className={`flex dark:bg-neutral-800 w-full dark:text-gray-100 bg-neutral-200 text-neutral-900 `}>
      ...
      <main className="flex-1  md:w-3/4 ml-1/4  sm:w-[100vw] px-4 md:px-10  pt-20 pb-8 md:py-8 flex   bg-gradient-to-r from-gradient-start to-gradient-end min-h-[100vh] ">
        ...
        {loading ? (
          <>
            ...
            /> */}
            <ImageGenerationSkeleton uploadedImage={uploadedImage} images={images} />
          </>
        ) : (
          <>
            <AnomalyBody
