...
const TrainingHistory = ({ selectedPrompt, setSelectedPrompt, darkTheme, setModelData, setShowModel, setModelsDetails, setDatasetsData, setTrainingSubmition }) => {
  ...
  const fetchProject = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/train/models/${username}`);
      if (!response.ok) {
  ...
  useEffect(() => {
    fetchProject(); // Fetch project data on component mount
  }, [username]);
  ...
  const toggleDrawer = () => {
    ...
    if (!isOpen) {
      ...
      fetchProject(); // Optionally refetch the project if needed
    }
  };
  ...
  const fetchModeldata = async (modelId) => {
    try {
      const response = await fetch(`${BACKEND_URL}/train/modelsdata/${modelId}`);
      if (!response.ok) {
        console.error('Failed to fetch models');
        return;
      }
      ...
      setModelsDetails(data.model_data);
      setDatasetsData(data.datasets_data);
      setTrainingSubmition(data.training_submition);
      setShowModel(true);
    } catch (error) {
      console.error('Error fetching project:', error);
    }
  ...
  return (
    <div
      className={`fixed top-0 pt-12 right-0 h-full shadow-lg p-4 transition-transform transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} z-30 w-80 dark:bg-neutral-900 dark:text-gray-100 bg-gray-100 text-gray-900 `}
    >
      <button
      ...
      <div className="h-full overflow-y-auto scrollbar-dark pt-4">
        <ul className="space-y-4">
          {prompts.map((prompt, index) => (
            ...
            >
              <button
                onClick={() => fetchModeldata(prompt.id)}
                className="flex items-center space-x-3 w-full text-base font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                <div className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full dark:bg-indigo-500 dark:text-white bg-indigo-100 text-indigo-500 `}>
                  <svg
                    className="w-5 h-5"
                    fill="none"
                    stroke="currentColor"
